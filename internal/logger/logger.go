...
type CustomFormatter struct {
	ForceColor bool // Whether to force colors even in non-terminal environments
}
...
func (f *CustomFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	// Build fields section: request_id priority, others sorted
	fields := ""

	// Output request_id first
	...
	// Sort and output remaining fields
	...
	// Build final output with colors
	if f.ForceColor {
...
// Fatalf outputs fatal level log using formatted string and exits program
func Fatalf(c context.Context, format string, args ...interface{}) {
	addCaller(GetLogger(c), 2).Fatalf(format, args...)
}

// CloneContext copies key information from context to new context
