...
func (e *AgentEngine) executeLoop(
	...
	if !state.IsComplete {
		...
		if err := e.streamFinalAnswerToEventBus(ctx, query, state, sessionID); err != nil {
			...
			state.FinalAnswer = "Sorry, I cannot generate a complete answer."
		}
		state.IsComplete = true
	}
...
func (e *AgentEngine) streamReflectionToEventBus(
	...
	reflectionPrompt := fmt.Sprintf(`Please evaluate the result of calling tool %s and decide on the next action.

Tool returned: %s

Think:
1. Does the result meet the requirements?
...
func (e *AgentEngine) streamFinalAnswerToEventBus(
	...
	finalPrompt := fmt.Sprintf(`Based on the above tool call results, please generate a complete answer for the user question.

User Question: %s

Requirements:
1. Answer based on actual retrieved content
2. Clearly mark information sources (chunk_id, document name)
3. Structure the answer in an organized way
4. If information is insufficient, honestly state so

Now please generate the final answer:`, query)
